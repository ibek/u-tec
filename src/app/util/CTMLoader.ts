...
import {CTM, CTMStream, CTMFile} from './CTM';

export class CTMLoader extends THREE.Loader {

    loadParts(url, callback, parameters) {
        ...
        var basePath = parameters.basePath ? parameters.basePath : this.extractUrlBase(
            url);
            ...
            if (xhr.readyState === 4) {
                ...
                if (xhr.status === 200 || xhr.status === 0) {
                    ...
                    for (var i = 0; i < jsonObject.materials.length; i++) {
                        ...
                        materials[i] = scope.createMaterial(jsonObject.materials[i],
                            basePath);
                    ...
                    if (parameters.useWorker) {
                        ...
                        worker.onmessage = function (event) {
                            ...
                            for (var i = 0; i < files.length; i++) {
                                ...
                                var e = Date.now();
                                console.log("model load time [worker]: " + (e - e1) +
                                    " ms, total: " + (e - s));
                        ...
                        for (var i = 0; i < offsets.length; i++) {
                            ...
                            var stream = new CTMStream(binaryData);
...
class Model extends THREE.BufferGeometry {
    ...
    constructor(file) {
        ...
        if (attrMaps !== undefined && attrMaps.length > 0 && attrMaps[0].name ===
            'Color') {
